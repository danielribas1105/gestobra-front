...
import { fetchBackend } from "@/functions/utils/fetch-backend";
...
export const useCurrentUser = () => {
  const { data: user, ...userQuery } = useQuery({
    ...
    queryFn: () => fetchBackend.get<User>(apiRoutes.user.me),
  });
...
export const useUpdateUser = () => {
  ...
  // Função que realiza a atualização do usuário no backend// Função que realiza a atualização do usuário no backend
  const updateUserData = async (userData: { user_id: User['id']; email: string; name: string }) => {
...
export const useChangePassword = () => {
  const client = useQueryClient();
  const changePassword = async (passwordUpdateData: { user_id: User['id'], password: string, confirmPassword: string }) => {
  ...
  const mutation = useMutation({
    mutationFn: async (passwordUpdateData: { user_id: User['id'], password: string, confirmPassword: string }) => changePassword(passwordUpdateData),
